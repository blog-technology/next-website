...
<pre className="line-numbers"><code className="language-javascript">{`// Example in Node.js (vulnerable)
const username = req.query.username;
const sql = \`SELECT * FROM users WHERE username = '${username}'\`;
db.query(sql);`}</code></pre><p>If an attacker enters:</p>
<pre className="line-numbers"><code className="language-bash">{`' OR '1'='1`}</code></pre><p>The query becomes:</p>
<pre className="line-numbers"><code className="language-sql">{`SELECT * FROM users WHERE username = '' OR '1'='1'`}</code></pre><p>✅ Which always returns all users.</p>
